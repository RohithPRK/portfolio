...
EXPERIENCE
${EXPERIENCES.map(exp => `${exp.role} — ${exp.company} (${exp.period}) | ${exp.location}\n${exp.responsibilities.map(r => `• ${r.point}`).join('\n')}`).join('\n\n')}

EDUCATION
${EDUCATION.map(ed => `• ${ed.degree} - ${ed.institution}`).join('\n')}

KEY PROJECTS
• Agentic MDM DCR Automation: LangGraph multi-agent workflow automating healthcare MDM change requests with human-in-the-loop review.
• AI Insights: Text-to-SQL agentic analytics platform converting natural language into live visualizations.
• Bleubird / AceETL: Orchestrated ETL/ELT pipelines connecting S3, SFTP, MySQL, Snowflake, and Dremio Data Lake.
• Leo Pharma HCP Segmentation: Enterprise Power BI reporting across 100+ tables and 50M+ pharmaceutical records.
• AceAI Chatbot: Enterprise RAG and Graph RAG chatbot with fine-tuned LLMs for clinical and enterprise queries.
      ...
      <div className="relative w-full max-w-4xl max-h-[90vh] bg-bg-card rounded-2xl border border-white/15 shadow-2xl flex flex-col overflow-hidden text-slate-100">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/10 bg-bg-darker/90">
          <div className="flex items-center gap-2">
            ...
            <span className="font-display font-semibold text-sm">Rohith Pandiarajan — Resume</span>
          </div>
