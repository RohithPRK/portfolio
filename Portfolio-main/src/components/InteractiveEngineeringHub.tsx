...
const SCENARIOS: PipelineScenario[] = [
  {
    ...
    steps: [
      ...
      { name: 'Hybrid Retrieval', node: 'RAG + Graph RAG', description: 'Dense vector search + sentence window retrieval', status: 'done', timeMs: 48 },
    ...
    metrics: { latency: '172ms', accuracy: '99.8%', stack: 'LangGraph • Django • Hugging Face' }
  },
  {
    id: 'etl-engine',
    label: 'AceETL Pipeline',
    category: 'Data Engineering',
    input: 'Ingest 50k unstructured records & auto-map schema',
    steps: [
      { name: 'Data Ingestion', node: 'Airbyte Worker', description: 'Concurrent batch streaming across S3, SFTP & MySQL', status: 'done', timeMs: 35 },
      { name: 'Schema Inference', node: 'Validator', description: 'Auto-detect types & missing field anomalies', status: 'done', timeMs: 29 },
      { name: 'Transformation', node: 'Python Engine', description: 'Vectorized normalization & duplicate cleanup', status: 'done', timeMs: 54 },
      { name: 'DB Ingestion', node: 'Snowflake & Dremio', description: 'ACID transaction commit with indexing', status: 'done', timeMs: 42 },
    ],
    output: 'Successfully cleaned & ingested 50,000 records with zero data drift.',
    metrics: { latency: '160ms', accuracy: '100% ACID', stack: 'Prefect • Airbyte • Snowflake' }
  },
  {
    id: 'mdm-dcr',
    label: 'MDM DCR Automation',
    category: 'Healthcare Data Automation',
    input: 'Evaluate healthcare provider change request & route decision',
    steps: [
      { name: 'DCR Intake', node: 'FastAPI Endpoint', description: 'Incoming change request capture & verify', status: 'done', timeMs: 18 },
      { name: 'Orchestrator Routing', node: 'LangGraph Orchestrator', description: 'Conditional routing & structured verification', status: 'done', timeMs: 44 },
      { name: 'Evidence Scoring', node: 'Snowflake + Web Evidence', description: 'Similarity-based address scoring engine', status: 'done', timeMs: 36 },
      { name: 'Human Review', node: 'Interrupt Checkpoint', description: 'Low-confidence cases paused for review', status: 'done', timeMs: 22 },
    ],
    output: 'Request routed and resolved with evidence-backed decisioning in under 120ms.',
    metrics: { latency: '120ms', accuracy: 'Automated', stack: 'LangGraph • Azure OpenAI • Snowflake' }
  }
];
          ...
          {activeTab === 'stack' && (
            ...
            >
              ...
              <div className="p-3 rounded-2xl glass-subtle space-y-2 border border-white/10">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {['Python (Django/FastAPI)', 'PostgreSQL', 'Snowflake', 'Prefect Workers', 'AWS', 'REST APIs'].map((item) => (
              ...
              <div className="p-3 rounded-2xl glass-subtle space-y-2 border border-white/10">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {['Power BI', 'DAX', 'Tableau', 'Row-Level Security', 'Playwright'].map((item) => (
          ...
          {activeTab === 'impact' && (
            ...
            >
              <div className="p-3 rounded-2xl glass-subtle border border-white/10 flex items-start gap-3">
                ...
                <div>
                  <div className="text-xs font-bold text-white font-display">AceETL Automated Pipelines</div>
                  <div className="text-[11px] text-slate-300 leading-relaxed mt-0.5">
                    Engineered orchestrated data ingestion across S3, SFTP, MySQL, Snowflake, and Dremio, processing enterprise-scale records reliably.
                  </div>
                </div>
              </div>
              ...
              <div className="p-3 rounded-2xl glass-subtle border border-white/10 flex items-start gap-3">
                ...
                <div>
                  <div className="text-xs font-bold text-white font-display">Agentic MDM Automation</div>
                  <div className="text-[11px] text-slate-300 leading-relaxed mt-0.5">
                    Architected a LangGraph agentic system automating healthcare MDM change requests with evidence-based decisioning and human review.
                  </div>
                </div>
              </div>
              ...
              <div className="p-3 rounded-2xl glass-subtle border border-white/10 flex items-start gap-3">
                ...
                <div>
                  <div className="text-xs font-bold text-white font-display">50M+ Records in Enterprise BI</div>
                  <div className="text-[11px] text-slate-300 leading-relaxed mt-0.5">
                    Delivered governed Power BI reporting across 100+ pharmaceutical tables and 50M+ records with row-level security.
                  </div>
                </div>
              </div>
            </motion.div>
