...
const INITIAL_ENTRIES: TerminalEntry[] = [
  {
    ...
    command: 'whoami',
    response: [
      'Rohith Pandiarajan — Senior Software Engineer at Data Aces, Chennai. 4+ years in AI & data engineering.',
      'Specializing in LangGraph agentic AI, RAG/Graph RAG pipelines, and enterprise data engineering & BI.'
    ],
    timestamp: '15:20:00'
  }
];
...
function getCommandResponse(input: string): string[] {
  ...
  if (!clean) {
    return [
      'Type any query or command above to learn about Rohith Pandiarajan.',
  ...
  if (clean === 'whoami' || clean.includes('who are you') || clean.includes('about')) {
    return [
      'Rohith Pandiarajan — Senior Software Engineer @ Data Aces in Chennai, building agentic AI & data platforms.',
      'Passionate about LangGraph orchestration, production RAG pipelines, and enterprise BI reporting.'
    ];
  }

  if (clean.includes('skill') || clean.includes('tech') || clean.includes('stack')) {
    return [
      'Core Tech: Python, LangGraph, CrewAI, Advanced RAG, Django, FastAPI, Snowflake, PostgreSQL, Vector DBs.',
      'Enterprise & Cloud: AWS, Azure AI, Power BI, DAX, ETL Pipelines, Playwright, Git.'
    ];
  }

  if (clean.includes('project') || clean.includes('work') || clean.includes('portfolio')) {
    return [
      'Flagship Projects: Agentic MDM DCR Automation (LangGraph), AI Insights (Text-to-SQL), and AceETL.',
      'Plus the AceAI RAG Chatbot and Leo Pharma HCP Segmentation. Explore all in the Projects page.'
    ];
  }

  if (clean.includes('experience') || clean.includes('job') || clean.includes('company') || clean.includes('data aces')) {
    return [
      'Senior Software Engineer at Data Aces (Chennai, 2023–Present) building agentic AI & data platforms.',
      'Previously Software Engineer at MCW (2022–2023) building data quality and BI reporting systems.'
    ];
  }

  if (clean.includes('education') || clean.includes('college') || clean.includes('degree') || clean.includes('mba')) {
    return [
      'MBA in Operations Management — Bharathidasan University (2021).',
      "Bachelor's in Mechanical Engineering — Anna University (2019)."
    ];
  }

  if (clean.includes('contact') || clean.includes('email') || clean.includes('hire') || clean.includes('reach')) {
    return [
      'Email: rohithkrishna98p@gmail.com • Base: Chennai, Tamil Nadu, India (IST UTC+5:30).',
  ...
  return [
    `Rohith Pandiarajan is a Full-Stack & AI Engineer at Data Aces building robust Python and RAG systems.`,
    `Feel free to reach out directly at rohithkrishna98p@gmail.com to collaborate or discuss opportunities.`
  ];
}
      ...
      >
        {history.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-center dark:text-slate-400 text-slate-600 space-y-1">
            ...
            <p>Terminal cleared. Type anything below to ask about Rohith.</p>
          </div>
        ) : (
          history.map((entry) => (
