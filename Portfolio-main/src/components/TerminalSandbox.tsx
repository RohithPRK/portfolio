...
export const TerminalSandbox: React.FC = () => {
  ...
  const [history, setHistory] = useState<Array<{ text: string; isUser?: boolean; isError?: boolean; isJson?: boolean }>>([
    ...
    { text: `Type 'help' or click suggestions below to inspect Rohith's engineering stack.` },
  ]);
...
def trigger_prefect_etl_flow(flow_name: str, parameters: dict) -> str:
  ...
  • projects     — Explore MDM DCR Automation, AI Insights, AceETL, AceAI Chatbot
  • skills       — List categorized languages, AI/ML tools & databases
  • experience   — View current role at Data Aces & recent deliverables
  ...
  • clear        — Clear terminal output`
      ...
      case 'projects':
        newHistory.push({
          text: `[1] Agentic MDM DCR Automation (LangGraph + Azure OpenAI + Snowflake + MCP)
[2] AI Insights (Text-to-SQL + Agentic AI + Multi-DB Routing)
[3] Bleubird / AceETL (Prefect + Airbyte + Snowflake + Dremio)
[4] AceAI Chatbot (RAG + Graph RAG + Hugging Face + Django + n8n)
[5] Leo Pharma HCP Segmentation (Power BI + DAX + SQL)`
        });
        break;
      case 'skills':
        newHistory.push({
          text: `• AI & Agents : LangGraph, CrewAI, Advanced RAG, MLflow, Fine-Tuning
• Languages   : Python, SQL
• Backend     : Django, FastAPI
• Databases   : Snowflake, PostgreSQL, Cassandra, Vector DBs, Dremio
• Cloud/Data  : AWS, Azure AI, Prefect, Airbyte, Power BI, Playwright`
        });
        break;
      case 'experience':
        newHistory.push({
          text: `🏢 Senior Software Engineer @ Data Aces (2023 – Present)
• Building agentic MDM automation with LangGraph orchestration & human review
• Designing text-to-SQL agentic analytics with dynamic visualization
• Engineering ETL/ELT pipelines across S3, SFTP, MySQL, Snowflake & Dremio
• Delivering enterprise Power BI reporting across 50M+ pharma records`
        });
        break;
      case 'status':
        newHistory.push({
          text: `🟢 SYSTEM STATUS: ONLINE
  ├─ LangGraph Orchestrator : ACTIVE (State: IDLE)
  ├─ Snowflake Connection   : CONNECTED (Latency: 18ms)
  ├─ MCP Tool Provider      : READY (Tools: 6 registered)
  ├─ Average Inference      : 410ms
      ...
      <div className="flex items-center justify-between px-4 py-2.5 bg-bg-darker/90 border-b border-white/10">
        <div className="flex items-center gap-2">
          ...
          <span className="ml-2 text-[11px] text-slate-400 font-medium hidden sm:inline-block">
            rohith@dev-node-01: ~/production-agent
          </span>
        </div>
